...
namespace MicrosoftGraphAspNetCoreConnectSample.Helpers
{
    public static class GraphService
    {
        ...
        public static async Task<string> GetDocument(GraphServiceClient graphClient, string itemId, HttpContext httpContext)
        {
            if (itemId == null) return JsonConvert.SerializeObject(new { Message = "itemId cannot be null." }, Formatting.Indented);

            try
            {
                
                //var s1 = graphClient.Workbooks["013O4DGKDW6AUCAM634BF3E4UZEXTGGJX4"].Workbook.Worksheets["Sheet1"];
                //var n1 = await s1.Range().Request().GetAsync();

                var s2 = graphClient.Me.Drive.Items["013O4DGKDW6AUCAM634BF3E4UZEXTGGJX4"].Workbook.Worksheets["Sheet1"];
                var n2 = await s2.UsedRange().Request().GetAsync();
            ...
            catch (ServiceException e)
            {
                switch (e.Error.Code)
                {
                    ...
                    case "ErrorItemNotFound":
                    case "itemNotFound":
                        return JsonConvert.SerializeObject(new { Message = $"itemId '{itemId}' was not found." }, Formatting.Indented);
                    case "ErrorInvalidUser":
                        return JsonConvert.SerializeObject(new { Message = $"The requested itemId '{itemId}' is invalid." }, Formatting.Indented);
                    case "AuthenticationFailure":
                        return JsonConvert.SerializeObject(new { e.Error.Message }, Formatting.Indented);
                    case "TokenNotFound":
                        await httpContext.ChallengeAsync();
